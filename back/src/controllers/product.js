...
export const listar_producto = async (req, res) => {
  ...
  try {
    const [result] = await conn.query("CALL listar_producto(?)", [id]);
    res.json(result[0]);
  } catch (error) {
    res.status(500).json({ message: error.message });
  }
...
export const crear_producto = async (req, res) => {
  ...
  try {
    const [result] = await conn.query("CALL insertar_producto(?,?,?)", [
      name,
      description,
      stock,
    ]);
...
export const actualizar_producto = async (req, res) => {
  ...
  try {
    const [result] = await conn.query("CALL editar_producto(?,?,?,?)", [
      id,
      name,
      description,
      stock,
